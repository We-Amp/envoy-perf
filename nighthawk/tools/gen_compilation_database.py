...
def modifyCompileCommand(target):
    ...
    # TODO(oschaaf): Had to add these, check that out.
    options = options.replace("-fno-canonical-system-headers ", "")
    options = options.replace("-Wno-free-nonheap-object ", "")
    options = options.replace("-Wunused-but-set-parameter ", "")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        'bazel_targets', nargs='*', default=["//source/..."])
